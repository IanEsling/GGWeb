...
<body>
<a href="#list-race" class="skip" tabindex="-1"><g:message code="default.link.skip.label"
...
<div id="list-race" class="content scaffold-list" role="main">
    ...
    <table>
        ...
        <tr>
            ...
            <g:sortableColumn property="numberOfRunners"
                              title="${message(code: 'race.numberOfRunners.label', default: 'Number Of Runners')}"/>
        ...
        <g:each in="${raceInstanceList}" status="i" var="raceInstance">
            <tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
                ...
                <td><g:link action="show"
                            id="${raceInstance.id}">${fieldValue(bean: raceInstance, field: "venue")}</g:link></td>
                
                <td>${fieldValue(bean: raceInstance, field: "time")}</td>

                <td>${fieldValue(bean: raceInstance, field: "numberOfRunners")}</td>

                <td>${fieldValue(bean: raceInstance, field: "bettable")}</td>
            </tr>
        </g:each>
        </tbody>
    </table>
